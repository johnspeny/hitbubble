...
bool Wall::showOnGameScene(const Vec2& position) {
	...
	//snMgr->addChild(m_bodySprite, graphical_layers::kBoardItems);
...
void Wall::defineBody(float x, float y) {
	...
	// top wall 0.01f
	bodyBuilder.newRectangleFixture(xPos / 2, yPos * 0.1f, b2Vec2(xPos / 2, yPos), 0,
	...
	//btmRight->setPosition((m_body->GetPosition().x + (xPos - 10) / GameUtils::PPM::kPpm) * GameUtils::PPM::kPpm, (m_body->GetPosition().y + (yPos * 0.36f) / GameUtils::PPM::kPpm) * GameUtils::PPM::kPpm);
	top->setPosition(xPos / 2, (m_body->GetPosition().y + (yPos) / GameUtils::PPM::kPpm) *
		GameUtils::PPM::kPpm);
	snMgr->addChild(top);

	// bottom wall
	// the height of the box is half the screen height (top on the screen and bottom off the screen)
	bodyBuilder.newRectangleFixture(xPos / 2, yPos * 0.2f, b2Vec2(xPos / 2, 0), 0,
		GameUtils::PPM::kPpm) // ground
		.restitution(0.2f)
		.denstity(1.0f)
		.categoryBits(kWallCategoryBits)
	...
	// left wall
	bodyBuilder.newRectangleFixture(1, visibleSize.height / 2, b2Vec2(0, yPos / 2), 0,
	...
	// right wall
	bodyBuilder.newRectangleFixture(1, visibleSize.height / 2, b2Vec2(xPos - 0, yPos / 2), 0,
...

	{
		...
		int numCircles = (visibleSize.height / GameUtils::PPM::kPpm) /
			(2 * circleRadius); // Number of circles
		...
		for (int i = 0; i < numCircles; ++i) {
			...
			circleFixtureDef.userData.pointer = reinterpret_cast<uintptr_t>(reinterpret_cast<void*>(WallFixtureType::LEFT_WALL));
			...
			left->setPosition(m_body->GetPosition().x * GameUtils::PPM::kPpm,
				yxPos * GameUtils::PPM::kPpm);
			//left->setPosition(m_body->GetPosition().x * GameUtils::PPM::kPpm, (m_body->GetPosition().y + (yPos * 0.37f) / GameUtils::PPM::kPpm) * GameUtils::PPM::kPpm);
	...
	// ------------right----------
	{
		...
		for (int i = 0; i < numCircles; ++i) {
			...
			right->setPosition(m_body->GetPosition().x +
				((visibleSize.width) / GameUtils::PPM::kPpm) * GameUtils::PPM::kPpm,
				yxPos * GameUtils::PPM::kPpm);
			snMgr->addChild(right, -1);
		}
	}

	bodyBuilder.newRectangleFixture(25.0f, 25.0f, b2Vec2(xPos - 10, yPos * 0.21f), 0.8f,
		GameUtils::PPM::kPpm) // right wall
		.categoryBits(kWallCategoryBits)
		.maskBits(kWallMaskBits)
	...
	btmRight->setPosition(
		(m_body->GetPosition().x + (xPos - 10) / GameUtils::PPM::kPpm) * GameUtils::PPM::kPpm,
		(m_body->GetPosition().y + (yPos * 0.21f) / GameUtils::PPM::kPpm) *
		GameUtils::PPM::kPpm);
	btmRight->setRotation(AX_RADIANS_TO_DEGREES(0.8f));
	snMgr->addChild(btmRight, graphical_layers::kRightBox);

	// circle bottom left
	bodyBuilder.newCircleFixture(b2Vec2(0, yPos * 0.2f), 50, GameUtils::PPM::kPpm)
		//.denstity(0.8f) // 0.25
		//.friction(1.0f) // 0.2
	...
	_bushSprite->setPosition(m_body->GetPosition().x * GameUtils::PPM::kPpm,
		(m_body->GetPosition().y + (yPos * 0.2f) / GameUtils::PPM::kPpm) *
		GameUtils::PPM::kPpm);
	...
	{
		auto visibleSize = Director::getInstance()->getVisibleSize();
		float xPos = visibleSize.width;
		float yPos = visibleSize.height;

		// Define the box dimensions
		float hx = 2.0f;     // Replace with your desired half-width
		float hy = 2.0f;     // Replace with your desired half-height
		float angle = 0.8f;  // Replace with your desired angle (in radians)
		float ppm = GameUtils::PPM::kPpm;   // Replace with your desired pixels per meter

		// Create a body definition
		b2BodyDef boxBodyDef;
		boxBodyDef.type = b2_dynamicBody; // Adjust the body type as needed
		boxBodyDef.position.Set(xPos / 2 / ppm, yPos * 0.8f / ppm); // Set the initial position

		// Create a Box2D body
		b2Body* boxBody = snMgr->getWorld()->CreateBody(&boxBodyDef);

		// Create the main box shape using SetAsBox
		b2PolygonShape boxShape;
		boxShape.SetAsBox(hx / ppm, hy / ppm, b2Vec2(0.0f, 0.0f), angle);

		// Create a fixture definition for the main box
		b2FixtureDef boxFixtureDef;
		boxFixtureDef.shape = &boxShape;
		boxFixtureDef.density = 1.0f; // Adjust the density as needed
		boxFixtureDef.friction = 0.3f; // Adjust the friction as needed
		boxFixtureDef.restitution = 0.5f; // Adjust the restitution as needed

		// Attach the fixture to the body
		boxBody->CreateFixture(&boxFixtureDef);

		// Create the left wall shape using SetAsBox
		b2PolygonShape leftWallShape;
		leftWallShape.SetAsBox(1.0f, 1.0f, b2Vec2(-(hx + 1.0f) / ppm, 0.0f), 0.0f);

		// Create a fixture definition for the left wall
		b2FixtureDef leftWallFixtureDef;
		leftWallFixtureDef.shape = &leftWallShape;
		leftWallFixtureDef.density = 1.0f; // Adjust the density as needed
		leftWallFixtureDef.friction = 0.3f; // Adjust the friction as needed
		leftWallFixtureDef.restitution = 0.5f; // Adjust the restitution as needed

		// Attach the left wall fixture to the body
		boxBody->CreateFixture(&leftWallFixtureDef);

		auto leftWallSprite = Sprite::create("CloseNormal.png");
		auto convertedPosition = Vec2((boxBody->GetPosition().x + (hx - 0.1f) / ppm) * ppm, boxBody->GetPosition().y * ppm);
		leftWallSprite->setPosition(convertedPosition);
		snMgr->addChild(leftWallSprite);
	}
	*/
}
