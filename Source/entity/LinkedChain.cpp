...
void LinkedChain::defineBody(float x, float y)
{
	...
	link = bodyBuilder.type(b2_dynamicBody)
    ...
    spriteF->setPosition(link->GetPosition().x * GameUtils::PPM::kPpm,
                         link->GetPosition().y * GameUtils::PPM::kPpm);
	...
	for (int i = 0; i < numberOfChains; i++)
	{
		...
		nextBodyPosition = nextBodyPosition - b2Mul(b2Rot(otherBodyAngle), b2Vec2(0, chain_hY * 0.75f / GameUtils::PPM::kPpm));
