...
GameOverLayer::GameOverLayer(const std::string& _mission) :
	m_mission{ _mission }
{
...
GameOverLayer* GameOverLayer::create(const std::string& _mission) {
	auto pRet = new GameOverLayer(_mission);
	if (pRet->init()) {
		pRet->autorelease();
	}
	else
...
bool GameOverLayer::init() {
	...
	// bg
	bg = ax::ui::ImageView::create("gameover.png", Widget::TextureResType::LOCAL);
	...
	listener->onTouchBegan = AX_CALLBACK_2(GameOverLayer::onTouchBegan, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);

	return true;
}

void GameOverLayer::setGameSceneReference(GameScene* gameScene) {
	m_gameScene = gameScene;
}

bool GameOverLayer::onTouchBegan(ax::Touch* touch, ax::Event* event) {
...
void GameOverLayer::displayCurrentMission()
{
	auto missionLabel = Label::createWithBMFont(fonts::hugmat_fnt, m_mission);
	missionLabel->setBMFontSize(40.0f);
	missionLabel->setColor(Color3B::RED);
	missionLabel->setPositionX(bg->getContentSize().width * 0.5f);
	missionLabel->setPositionY(bg->getContentSize().height * 0.6f);
	bg->addChild(missionLabel);
}