...
ToolBar::ToolBar() : m_leftItem{}, m_rightItem{}, m_middleItem{}
{
...
void ToolBar::addMiddleComponent(Widget* component, float scale)
{
    ...
    float padding     = 50.0f;
    float middleWidth = this->getContentSize().width - m_leftItem->getContentSize().width * scale -
                        m_rightItem->getContentSize().width * scale - (padding * 2);
...
void ToolBar::layoutStructure()
{
    ...
    this->setPosition(
        Vec2(m_origin.x + m_visibleSize.width / 2.0f, m_visibleSize.height - this->getContentSize().height * 0.5f));
